...
function setBg(key,node={}){
  ...
  rainSceneFactor=rainFactors[key] ?? .22;
...
function clearCharacter(){currentSprite=null;characterLayer.classList.add("hidden")}
...
function renderCharacter(node){
  ...
  const key=node.sprite||defaultKey;
  if(!key)return;
  setCharacter(key,node.spritePosition||"center",node.spriteShot||"medium",node.spriteMotion||"soft-in");
}

function renderPortrait(node){
  if(!npcCard)return;
  const key=node.portrait||"";
  if(!key||!PORTRAITS[key]||HEROINE_PORTRAIT_IDS.has(key)){npcCard.classList.add("hidden");return}
  const p=PORTRAITS[key];
  npcPortrait.src=p.src;
  npcName.textContent=node.portraitName||p.name;
  npcRole.textContent=node.portraitRole||p.role;
  npcCard.classList.remove("hidden","right","talking","large");
  if((node.portraitSide||"left")==="right")npcCard.classList.add("right");
  if(node.portraitLarge)npcCard.classList.add("large");
  if(node.speaker===p.name||node.portraitTalking)npcCard.classList.add("talking");
}

function setSceneMode(mode=""){
  document.body.classList.toggle("scene-car",mode==="car");
}
...
function loadSettings(){
  try{settings={...settings,...JSON.parse(localStorage.getItem(SETTINGS_KEY)||"{}")} }catch(error){}
  try{unlocks={...unlocks,...JSON.parse(localStorage.getItem(UNLOCK_KEY)||"{}")} }catch(error){}
  try{seen=new Set(JSON.parse(localStorage.getItem(SEEN_KEY)||"[]"))}catch(error){seen=new Set()}
  try{runtimeStats={...runtimeStats,...JSON.parse(localStorage.getItem(STATS_KEY)||"{}")} }catch(error){}
}

function saveUnlocks(){localStorage.setItem(UNLOCK_KEY,JSON.stringify(unlocks))}
function unlock(type,id){const arr=type==="cg"?unlocks.cg:unlocks.endings;if(!arr.includes(id)){arr.push(id);saveUnlocks()}}
